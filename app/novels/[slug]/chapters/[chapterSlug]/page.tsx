...
import { ChapterDetailsApiResponse } from "@/app/interfaces/story";
...
export default function ChapterDetailPage() {
  ...
  console.log("CH APTER SLUG", chapterSlug);
  ...
  const chapterDetails = useResourceStore(
      (s) => s.resources?.chapterDetails
    ) as ChapterDetailsApiResponse

    console.log("CHAPTER DEITALS: ", chapterDetails?.data)

  useEffect(() => {
    fetchResource("chapterDetails", "/chapter/detail", {
  ...
  }, [chapterSlug, fetchResource]);

  const allContents = chapterDetails?.data
  ...
  function getDisplayedContent() {
    ...
    //   );
    // }
    return allContents?.map((paragraph, index) => <p key={`full-${index}`}>{paragraph}</p>);
  }
