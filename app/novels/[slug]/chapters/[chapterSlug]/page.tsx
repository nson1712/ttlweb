...
async function fetchChapterDetails(slug: string, chapterSlug: string): Promise<ChapterApiResponse> {
...
async function fetchChapterContents(chapterId: number): Promise<ChapterDetailsApiResponse> {
...
export default function ChapterDetailPage({ params }: PageProps) {
  // 1. Dùng `use()` để “block” render cho tới khi data sẵn sàng
  const chapterDetails = use(fetchChapterDetails(params.slug, params.chapterSlug));
  const chapterContents = use(fetchChapterContents(Number(chapterDetails?.data?.id)));

  

  console.log("CHAPTER CONTENTS: ", chapterContents)

  return (
    <Suspense fallback={<LoaderIcon />}>
      <ChapterContent
        details={chapterDetails.data}
        contents={chapterContents?.data}
      />
    </Suspense>
  );
}
